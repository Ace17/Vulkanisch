...
const Vertex vertices[] = {
  ...
  { /*pos*/ -1.0f, -1.0f, +1.0f, /*uv*/ 0, 2 },
  { /*pos*/ -1.0f, +1.0f, +1.0f, /*uv*/ 2, 2 },
...
{
  auto vertShaderCode = loadFile("bin/src/hello-cube/shader.vert.spv");
  auto fragShaderCode = loadFile("bin/src/hello-cube/shader.frag.spv");
...
VulkanTexture createTexture(VkDevice device, VkPhysicalDevice physicalDevice, void* srcPixels, int width, int height)
{
  ...
  // This buffer is used as a transfer source for the buffer copy
  {
    ...
    // Allocate memory for the staging buffer
    {
      ...
      memAllocInfo.memoryTypeIndex = findMemoryType(physicalDevice, memReqs.memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
  ...
  auto TransferDataFromStagingBufferToTexture = [&](VkCommandBuffer cmdBuf) {
    ...
    setImageLayout(
          cmdBuf, texture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, range);
  };
  ...
  // Create sampler
  {
    ...
    vkCreateSampler(device, &info, nullptr, &texture.sampler);
  }
  ...
  // Create image view
  {
    ...
    info.format = format;
    info.components = { VK_COMPONENT_SWIZZLE_R, VK_COMPONENT_SWIZZLE_G, VK_COMPONENT_SWIZZLE_B,
      VK_COMPONENT_SWIZZLE_A };
    info.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1 };
    info.subresourceRange.levelCount = 1;
    info.image = texture.image;
    vkCreateImageView(device, &info, nullptr, &texture.view);
  }
...
class HelloCube : public IApp
{
  public:
  HelloCube(const AppCreationContext& ctx_)
      : ctx(ctx_)
  {
  ...
  void drawFrame(double time, VkFramebuffer framebuffer, VkCommandBuffer commandBuffer) override
  {
    ...
    clearColor.color.float32[0] = 0.4f;
    ...
    vkCmdBindDescriptorSets(
          commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0, nullptr);

    MyUniformBlock constants{};
    const float angle = time * 0.4;

    constants.model = rotateZ(angle * 0.3) * rotateY(angle * 0.2) * rotateX(angle * 0.25);
    constants.view = m_camera.mat;
    constants.proj = perspective(1.5, 4.0 / 3.0, 1, 100);
