...
class ApplicationHost
{
  ...
  void createSwapChain(VkPhysicalDevice physicalDevice, VkDevice device, VkSurfaceKHR surface)
  {
    ...
    const uint32_t maxImageCount = details.capabilities.maxImageCount > 0 ? details.capabilities.maxImageCount
                                                                          : UINT_MAX;
  ...
  void drawFrame()
  {
    ...
    VkResult result = vkAcquireNextImageKHR(
          device, swapchain, UINT64_MAX, frameSync[currFrameSync].availableForWriting, VK_NULL_HANDLE, &imageIndex);
