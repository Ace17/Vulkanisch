...
VkPipeline createColorPipeline(VkDevice device,
      VkPipelineLayout pipelineLayout,
...
VkPipeline createPostprocPipeline(VkDevice device,
      VkPipelineLayout pipelineLayout,
      VkExtent2D swapchainExtent,
      VkRenderPass renderPass,
...
// Perspective: Scene (set=0)
void setupDescriptorSet_MainScene(VkDevice device, VkDescriptorSet ds, VkBuffer uniformBuffer, VulkanTexture& shadowMap)
{
  assert(ds);
...
// PostProc (set=0)
void setupDescriptorSet_InputPicture(VkDevice device,
      VkDescriptorSet ds,
      VulkanTexture& inputPicture0,
      VulkanTexture& inputPicture1)
{
  assert(ds);
...
VulkanTexture createFramebufferForShadowMap(VkDevice device,
...
VulkanTexture createHdrOffscreenBuffer(VkDevice device,
      VkPhysicalDevice physicalDevice,
      VkExtent2D extent,
      VkRenderPass renderPass)
{
...
class FullDemo : public IApp
{
      ...
      : ctx(ctx_)
  {
    ...
    perspectivePipelineLayout = createPipelineLayout(
          ctx.device, { sceneDescriptorSetLayout, materialDescriptorSetLayout });

    postprocPipelineLayout = createPipelineLayout(ctx.device, { postprocDescriptorSetLayout });

    shadowMapPipeline = createShadowMapPipeline(ctx.device, perspectivePipelineLayout, shadowMapRenderPass);
    colorPipeline = createColorPipeline(ctx.device, perspectivePipelineLayout, ctx.swapchainExtent, colorRenderPass);

    thresholdPipeline = createPostprocPipeline(ctx.device, postprocPipelineLayout, ctx.swapchainExtent,
          postprocRenderPass, "bin/src/fulldemo/threshold.frag.spv");
    horzBlurPipeline = createPostprocPipeline(ctx.device, postprocPipelineLayout, ctx.swapchainExtent,
          postprocRenderPass, "bin/src/fulldemo/horzblur.frag.spv");
    vertBlurPipeline = createPostprocPipeline(ctx.device, postprocPipelineLayout, ctx.swapchainExtent,
          postprocRenderPass, "bin/src/fulldemo/vertblur.frag.spv");
    tonemapPipeline = createPostprocPipeline(ctx.device, postprocPipelineLayout, ctx.swapchainExtent, ctx.renderPass,
          "bin/src/fulldemo/tonemapping.frag.spv");

    hdrBuffer = createHdrOffscreenBuffer(ctx.device, ctx.physicalDevice, ctx.swapchainExtent, colorRenderPass);
    bloomBuffer[0] = createHdrOffscreenBuffer(ctx.device, ctx.physicalDevice, ctx.swapchainExtent, postprocRenderPass);
    bloomBuffer[1] = createHdrOffscreenBuffer(ctx.device, ctx.physicalDevice, ctx.swapchainExtent, postprocRenderPass);
    ...
    shadowMap = createFramebufferForShadowMap(
          ctx.device, ctx.physicalDevice, ShadowMapSize, ShadowMapSize, shadowMapRenderPass);
    ...
    postprocDescriptorSet_Bloom0_And_Bloom1 = createDescriptorSet(
          ctx.device, descriptorPool, postprocDescriptorSetLayout);
    setupDescriptorSet_InputPicture(
          ctx.device, postprocDescriptorSet_Bloom0_And_Bloom1, bloomBuffer[0], bloomBuffer[1]);
  ...
  void drawShadowMap(VkCommandBuffer commandBuffer,
        ...
        const Matrix4f& lightView,
        const Matrix4f& lightProj)
  {
    ...
    for (auto& mesh : vulkanMeshes)
    {
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, perspectivePipelineLayout, 0, 1,
  ...
  void drawMainScene(VkCommandBuffer commandBuffer,
        VkFramebuffer target,
        const Matrix4f& model,
        const Matrix4f& mvpLight)
  {
    ...
    for (auto& mesh : vulkanMeshes)
    {
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, perspectivePipelineLayout, 0, 1,
            &mainSceneDescriptorSet, 0, nullptr);

      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, perspectivePipelineLayout, 1, 1,
            &vulkanMaterials[mesh.material].descriptorSet, 0, nullptr);
  ...
  void drawFrame(double time, VkFramebuffer framebuffer, VkCommandBuffer commandBuffer) override
  {
    ...
    // threshold render pass: read from hdrBuffer, write to bloomBuffer[0]
    {
      ...
      vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, postprocPipelineLayout, 0, 1,
            &postprocDescriptorSet_Hdr_And_Bloom0, 0, nullptr);
    ...
    for (int k = 0; k < 4; ++k)
    {
      // Horz blur render pass: read from bloomBuffer[0], write to bloomBuffer[1]
      {
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, postprocPipelineLayout, 0, 1,
              &postprocDescriptorSet_Bloom0_And_Bloom1, 0, nullptr);
  ...
  VulkanTexture hdrBuffer{};
  VulkanTexture bloomBuffer[2]{};
