...
void setupDescriptorSet(VkDevice device,
      VkDescriptorSet ds,
      std::vector<VulkanTexture> inputTextures,
...
class Bloom : public IApp
{
      ...
      : ctx(ctx_)
  {
    ...
    thresholdPipeline = createPostprocPipeline(
          ctx.device, pipelineLayout, ctx.swapchainExtent, postprocRenderPass, "bin/src/bloom/threshold.frag.spv");
    horzBlurPipeline = createPostprocPipeline(
          ctx.device, pipelineLayout, ctx.swapchainExtent, postprocRenderPass, "bin/src/bloom/horzblur.frag.spv");
    vertBlurPipeline = createPostprocPipeline(
          ctx.device, pipelineLayout, ctx.swapchainExtent, postprocRenderPass, "bin/src/bloom/vertblur.frag.spv");
    tonemapPipeline = createPostprocPipeline(
          ctx.device, pipelineLayout, ctx.swapchainExtent, ctx.renderPass, "bin/src/bloom/tonemapping.frag.spv");
  ...
  void drawFrame(double time, VkFramebuffer framebuffer, VkCommandBuffer commandBuffer) override
  {
    // Color render pass: write to hdrBuffer[0]
    {
      ...
      vkCmdBindDescriptorSets(
            commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet[0], 0, nullptr);
    ...
    for (int k = 0; k < 4; ++k)
    {
      // Horz blur render pass: read from hdrBuffer[1], write to hdrBuffer[2]
      {
        ...
        vkCmdBindDescriptorSets(
              commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet[1], 0, nullptr);
      ...
      // Vert blur render pass: read from hdrBuffer[2], write to hdrBuffer[1]
      {
        ...
        vkCmdBindDescriptorSets(
              commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet[2], 0, nullptr);
    ...
    // Tone-mapping render pass: read from hdrBuffer[0] and hdrBuffer[1], write to the swapchain framebuffer
    {
      ...
      vkCmdBindDescriptorSets(
            commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet[3], 0, nullptr);
